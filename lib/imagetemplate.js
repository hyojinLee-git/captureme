...
module.exports={
    ...
    maintemplate:function(userinfo){
        ...
        for(var i=0; i<userinfo.length;i++){
            temp+=`
                <div class="container">
                    <a href="/${userinfo[i].id}">
                        <img class="thumbnail" src="${userinfo[i].representSrc}">
    ...
    //게시글개수의 배수일때 처리 필요(아무것도 없는 마지막페이지 형성됨)
    numbertemplate:function(userinfo){
        var temp=`
            <a href='http://localhost:8080/main'><input type="button" value="1"></a>`;
        var pagenum;
        if ((userinfo)%9==0){pagenum=parseInt((userinfo)/9);}
        else{pagenum=parseInt((userinfo)/9)+1;}
        for (var i=1; i<pagenum;i++){
            temp+=`
            <a href="/main/${i+1}"><input type="button" value="${i+1}"></input></a>`
        }
        return temp
    },
        ...
        for (var i=0; i<image.length;i++){
            ...
            <div class="mySlides fade">
                ...
                <div class="item description">
                    <a href="http://localhost:8080/photoApply/${id}" class="apply">
