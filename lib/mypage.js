...
var template = {
    ...
    application: function (request) {
        ...
        for (var i = 0; i < applicationList.length; i++) {
            ...
            var anotherData=readFileSync(`userInfo/${application.id}.json`, 'utf8');
            requesterInfo=JSON.parse(anotherData);
            tpl += `
            <li class="application">
                <img src="${requesterInfo.photoInfo.profileSrc}" class="picture">
                <div class="profileDescription">
                    <div class="userName">${requesterInfo.account.name}</div>
                    <div>${application.time} 까지</div>
                    <div>${application.location} 에서</div>
                    <div>${requesterInfo.account.phoneNumber}</div>
                </div>
                <div class="applicationAcception">
                    <button><a href='http://localhost:8080/mypage/applicationList/${i}/accept'>수락</a></button>
                    <button><a href='http://localhost:8080/mypage/applicationList/${i}/decline'>거절</a></button>
                </div>
            </li>`;
        }
        tpl += `</ul>`;
    ...
    acception: function (request) {
        ...
        for (var i = 0; i < acceptionList.length; i++) {
            ...
            var anotherData=readFileSync(`userInfo/${acception.id}.json`, 'utf8');
            requesterInfo=JSON.parse(anotherData);
            tpl += `
            <li class="application">
                <img src="${requesterInfo.photoInfo.profileSrc}" class="picture">
                ...
                <div class="applicationAcception">
                    <button><a href='http://localhost:8080/complete/acceptionList/${i}'>완료</a></button>
                </div>
            </li>`;
        }
        tpl += `</ul>`;
