...
module.exports={
    html:function(request,response){
        ...
        db.query(`SELECT test.id,name,profileSrc,representSrc FROM test LEFT JOIN test_image ON test_image.id=test.id WHERE NOT representSrc is NULL LIMIT 0,5;`,function(err,userinfo){
            if(err){throw err}
    ...
    pagehtml:function(request,pagenum,response){
        ...
        var startnum=(pagenum-1)*5; //(pagenum-1)*게시글 개수
        var nextpage=pagenum+1;
        var prevpage=pagenum-1
        db.query(`SELECT test.id,name,profileSrc,representSrc FROM test LEFT JOIN test_image ON test_image.id=test.id WHERE NOT representSrc is NULL`,function(err,boardnum){
        db.query(`SELECT test.id,name,profileSrc,representSrc FROM test LEFT JOIN test_image ON test_image.id=test.id WHERE NOT representSrc is NULL LIMIT ${startnum},5;`,function(err,userinfo){
            if(err){throw err}
                    var data= `
                    <html lang="en">
            <head>
