...
module.exports={
    html:function(request,response){
        db.query(`SELECT test.id,name,profileSrc,representSrc FROM test LEFT JOIN test_image ON test_image.id=test.id WHERE NOT representSrc is NULL`,function(err,boardnum){
            if (err){throw err;}
            db.query(`SELECT test.id,name,profileSrc,representSrc FROM test LEFT JOIN test_image ON test_image.id=test.id WHERE NOT representSrc is NULL ORDER BY test.No DESC LIMIT 0,9;`,function(err2,userinfo){
                if(err2){throw err2;}
                var data= `
                    <html lang="en">
                    <head>
    ...
    searchpage:function(request,id,response){
        db.query(`SELECT test.id,name,profileSrc,representSrc FROM test LEFT JOIN test_image ON test_image.id=test.id WHERE name=?`,[id],function(err,boardnum){   if(err){throw err;}
            db.query(`SELECT test.id,name,profileSrc,representSrc FROM test LEFT JOIN test_image ON test_image.id=test.id WHERE name=?;`,[id],function(err2,userinfo){
                if(err2){throw err2;}
                var data= `
                    <html lang="en">
                    <head>
